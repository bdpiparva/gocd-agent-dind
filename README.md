...
[GoCD agent](https://www.gocd.org) docker image based on alpine 3.5.

# Issues, feedback?

Please make sure to log them [Here](https://github.com/bdpiparva/gocd-agent-dind).

# Docker in docker 
*_Note: Please follow the [Usage](#usage) section to start GoCD agent._*

In order to use this as a docker in docker mode you have to start container in privileged mode as mentioned in following example -

```
docker run -d --privileged -e GO_SERVER_URL=... bdpiprava/gocd-agent-dind:0.0.1
```

1. You can install docker version of you choice using following command -
    ```$bash
    sudo dvm install 17.09.1
    ```
    Note: Supported versions are listed [Here](https://download.docker.com/linux/static/stable/x86_64)
...
```
docker run -d -e GO_SERVER_URL=... bdpiprava/gocd-agent-dind:0.0.1
```
...
```
docker run -itd -e GO_SERVER_URL=https://$(docker inspect --format='{{(index (index .NetworkSettings.IPAddress))}}' angry_feynman):8154/go bdpiprava/gocd-agent-dind:0.0.1
```
OR
...
```
docker run -itd -e GO_SERVER_URL=https://<ip_of_host_machine>:$(docker inspect --format='{{(index (index .NetworkSettings.Ports "8154/tcp") 0).HostPort}}' angry_feynman)/go bdpiprava/gocd-agent-dind:0.0.1
```
...
docker run -d \
        ...
        bdpiprava/gocd-agent-dind:0.0.1
```
...
```
docker run -v /path/to/godata:/godata -v /path/to/home-dir:/home/go bdpiprava/gocd-agent-dind:0.0.1
```
...
```
docker run -e GO_AGENT_SYSTEM_PROPERTIES="-Dfoo=bar" bdpiprava/gocd-agent-dind:0.0.1
```
